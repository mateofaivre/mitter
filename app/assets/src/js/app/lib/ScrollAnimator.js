...
export default class ScrollAnimator {
	...
	updatePosition(fast) {
		...
		if (percent <= 0 && this.isMin !== true) {
			...
			// this.options.target.trigger("lesanimals:scrollanimator:min");
		}
		...
		if (percent >= 1 && this.isMax !== true) {
			...
			// this.options.target.trigger("lesanimals:scrollanimator:max");
		}
		...
		if ((this.isMin === true || this.isMax === true) && this.isInside !== false) {
			...
			// this.options.target.trigger("lesanimals:scrollanimator:outside");
		}
		...
		// Check if we are inside
			...
			// this.options.target.trigger("lesanimals:scrollanimator:inside");
		}
	...
	update() {
		...
		// this.options.target.trigger("lesanimals:scrollanimator:update", [this.coords]);
	...
	destroy() {
		...
		window.removeEventListener('scroll', this.scrollHandler);
		window.removeEventListener('resize', this.update);
		// window.removeEventListener('lesanimals:scrollanimator:tick', this.updatePosition);
		if (this.options.tween) {
			this.options.tween.paused(true);
			this.options.tween.progress(this.options.defaultProgress);
			this.options.tween.kill();
